...
- Public app code lives in `app/`, shared UI in `components/`, shared logic and registries mostly in `lib/`, content/data in `content/` and `data/`.
- Canonical agent guidance now lives in `agent-docs/` with `AGENTS.md` as the router.

## Known Architecture Risks

- `components/ServiceLandingPage.tsx` carries common service rendering, service-specific sections, schema assembly, and CTA logic in one large module.
