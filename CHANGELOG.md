...
- Deployment: Vercel preview consent and event QA is required before squash merge; production follows the validated merge through the existing Git integration.
- Summary:
...
- Summary:
  ...
  - Added regression coverage for source metadata, approved testimonial proof, outcome wording, and the unchanged five-link set.
- Files changed:
  - `features/content-sources/index.ts`
  - `data/caseStudies.ts`
  - `tests/rebel-bunny-case-study-refresh.test.ts`
  - `CHANGELOG.md`
- Verification:
...
- Summary:
  - Updated stable `@types/react-dom` from `19.2.4` to `19.2.5`, adding the upstream optional diagnostic argument for the React canary `browser()` type.
  - Limited the lockfile delta to the single direct dev dependency record without changing runtime React, dependencies, peer requirements, package count, or install scripts.
  - Deferred engine-incompatible `sanitize-html` `2.17.7`, the broader Anthropic SDK 0.x line, ESLint 10, TypeScript 7, and Node types 26 to separate compatibility reviews.
- Files changed:
  - `package.json`
  - `package-lock.json`
  ...
  - `CHANGELOG.md`
- Verification:
  - Node `24.19.0`, npm `11.18.0`, `npm ci`, `npm audit`, package-diff inspection, install-script review, and `git diff --check`: passed; audit reports 0 vulnerabilities.
  - `npm run lint`, `npm run typecheck`, `npm run validate:content`, and `npm run audit:shopify-claims`: passed.
  - `npm run test`: passed, 30 test files and 124 tests.
  - `NEXT_PUBLIC_SITE_URL=https://hydrogenexpert.co npm run build`: passed with 76 static pages generated.
- Manual follow-up: None.
...
- Date: 2026-08-23
- PR: [#123 Update Next.js to 16.3.2](https://github.com/emremutlu08/hydrogenexpert/pull/123)
- Branch: `codex/daily-deps-20260823`
- Deployment: Vercel preview validation is required before squash merge; production follows the validated merge without a separate manual deploy.
- Summary:
  - Updated Next.js, `@next/third-parties`, and `eslint-config-next` from `16.3.1` to stable `16.3.2`, an official non-prerelease bug-fix backport.
  - Limited the lockfile delta to 13 version-coupled Next.js records without changing package counts, React, the declared Node range, peer dependencies, duplicate runtimes, or install scripts.
  - Deferred engine-incompatible `sanitize-html` `2.17.7`, the broader Anthropic SDK 0.x line, ESLint 10, TypeScript 7, and Node types 26 to separate compatibility reviews.
- Files changed:
  - `package.json`
  - `package-lock.json`
  ...
  - `CHANGELOG.md`
- Verification:
  - Node `22.22.3`, npm `11.18.0`, `npm ci`, `npm audit --json`, dependency-tree inspection, install-script review, and `git diff --check`: passed; audit reports 0 vulnerabilities across 555 packages.
  - `npm run lint`, `npm run typecheck`, `npm run validate:content`, and `npm run audit:shopify-claims`: passed.
  - `npm run test`: passed, 30 test files and 124 tests.
  - `NEXT_PUBLIC_SITE_URL=https://hydrogenexpert.co npm run build`: passed with Next.js `16.3.2`; 76 static pages generated.
- Manual follow-up: None.
...
- Deployment: Vercel preview validation is required before squash merge; production follows the validated merge without a separate manual deploy.
- Summary:
  - Updated ESLint from `9.39.4` to stable `9.39.5` on the existing Next-compatible ESLint 9 line.
  - Aligned `@eslint/js` to `9.39.5` and `@eslint/eslintrc` to `3.3.6` without changing the existing `acorn` or install-script package set.
  - Deferred unrelated test tooling, engine-incompatible, major, and broader Anthropic SDK updates to independent reviews.
- Files changed:
  - `package.json`
  - `package-lock.json`
  - `agent-docs/HYDROGEN.md`
...
- Summary:
  ...
  - Deferred styling, test-tooling, engine-incompatible, major, and 0.x SDK updates to independent reviews.
- Files changed:
  - `package.json`
  - `package-lock.json`
  - `agent-docs/HYDROGEN.md`
